...
const TasbihDigital = () => {
  ...
  const incrementCount = () => {
    setCount(prevCount => {
      ...
      if (newCount > target) {
        // If target is exceeded, reset to 1 and show notification
        Swal.fire({
          title: 'Selesai!',
        ...
        return 1; // Reset to 1 after reaching target
      }
      return newCount;
    });
  };
  ...
  const handleTargetChange = (e) => {
    ...
    setTarget(val === '' ? 0 : Math.max(0, parseInt(val, 10) || 0)); // Ensure target is not negative
  };

  const handleTargetBlur = () => {
    if (target === 0) setTarget(33); // Reset to default if left empty or 0
  ...
  return (
    <div className="container-custom py-6">
      ...
      <motion.div
        ...
        className="bg-white rounded-2xl shadow-lg overflow-hidden flex flex-col items-center justify-center p-6 md:p-8 max-w-lg md:max-w-xl lg:max-w-2xl xl:max-w-3xl mx-auto"
      >
        <div className="bg-gradient-to-r from-emerald-600 to-emerald-500 p-5 text-white w-full text-center rounded-t-2xl -mt-6 -mx-6 mb-6">
        ...
        <motion.div
          ...
          style={{ userSelect: 'none' }} // Mencegah teks terpilih saat diklik cepat
        >
        ...
        <div className="mt-6 text-center w-full">
          ...
          {showTargetInput ? (
            <input
              type="number"
              value={target === 0 ? '' : target} // Show empty string if target is 0
              onChange={handleTargetChange}
              onBlur={() => {
                if (target === 0) setTarget(33); // Reset to default if left empty
                setShowTargetInput(false);
              }}
      ...
      <motion.div
        ...
        className="mt-8 bg-white rounded-2xl shadow-lg p-6 max-w-4xl mx-auto"
      >
        <div className="bg-gradient-to-r from-blue-600 to-blue-500 p-5 text-white w-full text-center rounded-t-2xl -mt-6 -mx-6 mb-6">
